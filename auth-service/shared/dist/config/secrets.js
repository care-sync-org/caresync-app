...
async function initSecrets() {
    ...
    let response;
    try {
        response = await client.send(new client_secrets_manager_1.GetSecretValueCommand({ SecretId: secretName }));
    }
    catch (err) {
        const msg = `Failed to retrieve secrets from Secrets Manager (${secretName}). ` +
    ...
    for (const key of required) {
        if (!secrets[key]) {
            throw new Error(`Secret "${secretName}" is missing required field: "${key}". ` +
